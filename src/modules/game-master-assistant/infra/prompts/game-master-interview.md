...
The transcript is the full conversation so far: the original goal, prior Game Master messages, prior User replies, and the latest User reply.

Return `readinessStatus: "ready_to_generate"` only when the conversation so far has enough concrete detail for a specific, actionable Adventure. Before readiness, cover these signals:
...
Coverage means the conversation so far already contains enough concrete User-provided detail to use that signal in planning. Asking about a signal does not cover it. If the User gives a vague or partial answer, keep that signal uncovered and ask another sharper one-at-a-time follow-up for the same signal until it is usable.
...
- `coveredSignals` must include every signal with enough concrete evidence in the conversation so far for planning, even when your next question targets a different uncovered signal. Do not mark a signal covered merely because `messageToUser` asks about it.
...
- If `summaryDelta` says or summarizes that a readiness signal is known, that signal must also be included in `coveredSignals`.
