...
- Do not combine “what have you tried?” with “what got in the way?” Ask one first, then follow up only if needed. When asking either one, include concrete options or quoted answer shapes so the User can answer without doing diagnostic work.
...
- Most questions should include concrete options or answer-shaped examples the User can copy, select, or correct. When asking an open-ended “what are you missing / what do you need / what would help?” question, use quoted first-person answer shapes, such as “I need a menu plan,” “I’m unsure how to time four courses,” “I need knife-skill practice,” or “I need a shopping list under $120.”
...
- `existingInventory`: user-controlled practical readiness artifacts, documents, accounts/access, routines, environment setup, tools, materials, or resources the User already has available, such as supplies, statements, spreadsheets, apps, documents, knowledge, habits, equipment, or learning materials. Do not treat people, groups, assumed helpers, relationships, or fantasy loot as inventory. If support from people may matter, ask about it separately as support/context, not as inventory.
...
- `firstMilestoneReadiness`: enough detail to pick a realistic first milestone. The User does not need to explicitly name the first milestone; this signal is covered when the transcript provides enough target outcome, timeline/scale, current stage, constraints, and safety/usefulness cues to choose a sensible first win. Do not require the User to name the first milestone explicitly, and do not require every other readiness signal to be covered before marking this covered.
...
3. Reconcile signals after drafting `summaryDelta`:
   ...
   - If the transcript or `summaryDelta` says the User has, owns, can access, or has available a concrete tool, document, account/access, routine, material, setup, or user-controlled resource, set `existingInventory` to `true` and do not ask another inventory question.
   ...
   - If it contains enough target outcome, timeline/scale, current stage, constraints, and safety/usefulness cues to choose a sensible first win, set `firstMilestoneReadiness` to `true` even if the User did not explicitly name the first milestone. Do not require known missing resources before covering first-milestone readiness; missing resources can remain separately uncovered.
...
7. For `not_ready` questions:
   ...
   - Include concrete choices or quoted answer-shaped examples tailored to that signal. For `likelyMissingResources`, strongly prefer quoted first-person answer shapes, such as “I need a menu plan,” “I’m missing serving dishes,” or “I need a friend to help with cleanup.” Do not use only broad categories.
   - If `pastFriction` is uncovered and materially applicable, ask either what they tried before or what got in the way; do not combine both unless one was already answered. Include concrete friction choices or quoted answer shapes, such as “I tried an app but stopped updating it,” “I kept losing momentum,” or “unexpected issues knocked me off track.”
